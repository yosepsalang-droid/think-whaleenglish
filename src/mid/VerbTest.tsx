...
// 💡 첨부해주신 image_8ae101.png 파일의 데이터를 기반으로 한 예시입니다.
// 원장님께서 5개씩 끊어서 day 번호를 매겨주시면 됩니다.
const VERB_DATA = [
  ...
  { day: 6, kor: '~이다, 있다', base: 'be', past: 'was', pp: 'been' }, // 💡 참고: 테스트 편의를 위해 'was'만 입력하도록 세팅했습니다.
  ...
  // Day 24
  { day: 24, kor: '눕다', base: 'lie', past: 'lay', pp: 'lain' }, // 💡 참고: '거짓말하다'는 규칙 동사(lied)입니다.
  { day: 24, kor: '(옷감을) 짜다, 엮다', base: 'weave', past: 'wove', pp: 'woven' },
  { day: 24, kor: '밟다, 걷다', base: 'tread', past: 'trod', pp: 'trodden' },
  { day: 24, kor: '줄어들다', base: 'shrink', past: 'shrank', pp: 'shrunk' },
...
export default function VerbTest({ onBack, studentId = "ST_TEST", studentName = "테스트학생" }: VerbProps) {
  // 화면 전환 상태: 'SELECT'(범위선택) -> 'PRACTICE'(연습) -> 'TEST'(시험) -> 'RESULT'(결과)
  ...
  const handleStartTest = () => {
    // 💡 연습 모드에서 시험 모드로 넘어갈 때, 단어 순서를 무작위로 섞습니다.
      ...
      {step === 'SELECT' && (
        <div style={{ padding: '30px 20px', backgroundColor: '#f8f9fa', borderRadius: '16px', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', alignItems: 'center', marginBottom: '30px' }}>
            <select value={startDay} onChange={(e) => setStartDay(e.target.value)} style={selectStyle}>
              <option value="">시작 Day</option>
              {days.map(d => <option key={`start-${d}`} value={d}>Day {d}</option>)}
            </select>
            <span style={{ fontWeight: 'bold' }}>~</span>
            <select value={endDay} onChange={(e) => setEndDay(e.target.value)} style={selectStyle}>
      ...
      {step === 'TEST' && (
        <div style={{ padding: '30px 20px', backgroundColor: 'white', border: '1px solid #eee', borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
          ...
          <div style={{ textAlign: 'center', margin: '20px 0 30px 0' }}>
            ...
            <button onClick={speakCurrentVerbs} style={{ padding: '8px 16px', backgroundColor: '#eef2ff', color: '#4f46e5', border: 'none', borderRadius: '20px', fontSize: '14px', fontWeight: 'bold', cursor: 'pointer' }}>
              🔊 발음 듣기
            </button>
          </div>
